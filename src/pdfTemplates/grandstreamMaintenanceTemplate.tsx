...
interface GrandstreamMaintenanceFormData {
  customerName: string;
...
export async function generateGrandstreamMaintenancePDF(
  formData: GrandstreamMaintenanceFormData,
  savePath: string
): Promise<string> {
  ...
  const title2 = "[GRANDSTREAM TELEPHONE SYSTEM]";
  ...
  page.drawText(`[${formData.systemModel === 'UCM6301' ? 'X' : ' '}] UCM6301 IP PBX`, { x: 160, y, size: 12, font: calibriFont, color: blackColor });
  page.drawText(`[${formData.systemModel === 'UCM6202' ? 'X' : ' '}] UCM6202 IP PBX`, { x: 320, y, size: 12, font: calibriFont, color: blackColor });
  page.drawText("RELEASE:", { x: 480, y, size: 12, font: calibriBoldFont, color: blackColor });
  page.drawText(` ${formData.release || ''}`, { x: 550, y, size: 12, font: calibriBoldFont, color: primaryBlue });
  y -= 25;
  ...
  const faultySubsequentLineWidth = width - 50 - 50;
  ...
  const maintenanceItems = [
    ...
    { id: 2, desc: "System Restart", hasEquipped: false },
    { id: 3, desc: "Clean Operator Console / User Phones", hasEquipped: false },
    { id: 4, desc: "Test / Diagnosis of Phones", hasEquipped: false },
    { id: 5, desc: "Check Automatic Power Back-up", hasEquipped: false },
    { id: 6, desc: "Check the condition of IP PBX", hasEquipped: false },
    { id: 7, desc: "Check the condition of GSM – SIP Gateway", hasEquipped: false },
    { id: 8, desc: "Check condition of analogue trunk lines", hasEquipped: true },
    { id: 9, desc: "Check condition of SIP Trunk lines", hasEquipped: true },
    { id: 10, desc: "Check all Indoor and Outdoor Wi-Fi Access Points(8)", hasEquipped: false },
  ...
  systemAppsData.forEach(app => {
    const remark = mapRemark(formData.systemApps[app.key], 0); // 0 for non-done type
  ...
  page.drawText("Check Extension Lines:", { x: 70, y, size: 12, font: calibriBoldFont, color: blackColor });
  y -= 18;

  // First line with extension types - no dotted lines
  const wifiIpPhoneText = `Equipped Wi-Fi IP Phone: `;
  page.drawText(wifiIpPhoneText, { x: 50, y, size: 12, font: calibriFont, color: blackColor });
  page.drawText(formData.extensionLines.wifiIpPhone || '', { x: 50 + calibriFont.widthOfTextAtSize(wifiIpPhoneText, 12), y, size: 12, font: calibriBoldFont, color: primaryBlue });

  const wifiCordlessPhoneText = `Equipped Wi-Fi Cordless Phone: `;
  page.drawText(wifiCordlessPhoneText, { x: 300, y, size: 12, font: calibriFont, color: blackColor });
  page.drawText(formData.extensionLines.wifiCordlessPhone || '', { x: 300 + calibriFont.widthOfTextAtSize(wifiCordlessPhoneText, 12), y, size: 12, font: calibriBoldFont, color: primaryBlue });
  y -= 18;
  ...
  // Replace it with:
  const actualFooterHeight = 40; // Height for footer content
  const minSpaceAboveFooter = 20;
  const signatureBottomY = customerContainerY;

  let footerY = Math.max(signatureBottomY - minSpaceAboveFooter - actualFooterHeight, 45);
...
function mapRemark(value: string, itemId?: number): string {
  ...
  // For items 5-11 and System Applications, use Checked/Not Checked
  switch (value) {
    case 'done':
    case 'checked':
    case 'Checked':
